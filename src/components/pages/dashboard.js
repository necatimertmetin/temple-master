...
const Dashboard = () => {
  ...
  const templatesTest = [
    {},
    {
      id: 1,
      name: "Template 1",
      description: "Bu birinci şablonun açıklamasıdır.",
    },
    {
      id: 2,
      name: "Template 2",
      description: "Bu ikinci şablonun açıklamasıdır.",
    },
    {
      id: 1,
      name: "Template 3",
  ...
  const handleClick = (id) => {
    // Navigate to the new URL with templateId
          ...
          {templates?.map((template, index) => (
            <div key={index} className="grid-item">
              {index === 0 ? (
                ...
                <div
                  onClick={() => {
                    handleClick(template?.TemplateId);
                  }}
                  style={{ cursor: "pointer" }}
                >
                  <img
                    className="grid-item-img"
                    style={{ filter: `hue-rotate(${index * 100}deg)` }}
                    src={PlaceholderImage}
                    alt="template gorseli bulunamadi"
                  />
                  <div>
